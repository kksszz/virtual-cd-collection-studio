...
final class TimerNotice {
    ...
    static String summary(android.content.SharedPreferences prefs){
        long at=prefs.getLong("lastStoppedAt",0);if(at<=0)return "タイマー終了の記録はまだありません。";
        ...
        return time+"\n自動停止タイマー（"+(prefs.getLong("lastStopLimit",0)/60000)+"分）により再生を停止しました。";
    }
    static void announce(Context context){
        String text="自動停止タイマーの時間になりました。再生を停止しました。";
        try{Toast.makeText(context,text,Toast.LENGTH_LONG).show();}catch(RuntimeException ignored){}
        try{
            NotificationManager manager=context.getSystemService(NotificationManager.class);
            NotificationChannel channel=new NotificationChannel(CHANNEL,"自動停止タイマーの終了",NotificationManager.IMPORTANCE_DEFAULT);
            channel.setDescription("タイマー終了の通知。短い終了音と、次回起動時の終了理由を表示します。");channel.setSound(null,null);channel.enableVibration(false);manager.createNotificationChannel(channel);
            ...
            if(manager.areNotificationsEnabled())manager.notify(ID,new Notification.Builder(context,CHANNEL).setSmallIcon(android.R.drawable.ic_lock_idle_alarm).setContentTitle("自動停止タイマーで終了しました")
    ...
    static void showPending(Activity activity){
        ...
        new AlertDialog.Builder(activity).setTitle("前回はタイマーで終了しました").setMessage(summary(prefs)+"\n\nクラッシュではなく、設定時間による自動停止です。")
            .setPositiveButton("確認",(d,w)->{if(prefs.getLong("lastStoppedAt",0)==at)prefs.edit().putBoolean("stopNoticePending",false).apply();activity.getSystemService(NotificationManager.class).cancel(ID);}).show();
    }
}
