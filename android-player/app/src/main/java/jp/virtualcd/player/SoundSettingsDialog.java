...
final class SoundSettingsDialog {
    private static final String[] MODES={"OFF","軽め","標準","強め","劇的（強調）","HDR風（実験）"};
    private static final String[] PRESETS={"フラット","低音強調","高音強調","ボーカル","ロック","カスタム"};
    ...
    private SoundSettingsDialog(Activity activity){
        ...
        LinearLayout header=new LinearLayout(activity);root.addView(header);TextView title=text("音の設定",21);header.addView(title,new LinearLayout.LayoutParams(0,dp(48),1));
        Button close=new Button(activity);PlayerStyle.button(close);close.setText("閉じる");close.setOnClickListener(v->dialog.dismiss());header.addView(close,new LinearLayout.LayoutParams(dp(64),dp(44)));
        ...
        note("再生中に反映・自動保存。元の音楽ファイルは変更しません。");
        body.addView(text("再生速度・ピッチ",16));
        speed=slider("速度",30,(store.speedPercent()-50)/5,false,v->saveTuning(),v->String.format(java.util.Locale.ROOT,"%.2f×",(50+v*5)/100.0));
        pitch=slider("ピッチ",24,store.pitchSemitones()+12,false,v->saveTuning(),v->String.format(java.util.Locale.ROOT,"%+d 半音",v-12));
        Button resetTuning=new Button(activity);PlayerStyle.button(resetTuning);resetTuning.setText("速度・ピッチを標準に戻す");body.addView(resetTuning,new LinearLayout.LayoutParams(-1,dp(44)));
        resetTuning.setOnClickListener(v->{binding=true;speed.setProgress(10);pitch.setProgress(12);binding=false;saveTuning();});
        note("速度：0.50〜2.00倍（0.05刻み）／ピッチ：±12半音。独立して調整できます。原音忠実モードとは別設定です。標準は1.00倍・0半音です。");
        gapless=check("ギャップレス再生",store.gapless(),false);note("ON：連続再生。OFF：曲ごとに再開します。音源内の無音は除去しません。");
        faithful=check("原音忠実モード",initial.faithful,false);note("ON：以下の補正をバイパス（設定値は保持）。Androidの出力経路を含むビットパーフェクトを保証する機能ではありません。");
        body.addView(text("音質向上（リアルタイム）",16));mode=spinner(MODES,initial.mode);effects.add(mode);
        normalize=check("音量ノーマライズ",initial.normalize,true);note("RMS基準で曲の音量差を緩やかに補正。LUFS解析／ReplayGainではありません。");
        clarity=check("小音量クリア",initial.clarity,true);
        eq=check("10バンドEQ ON",initial.eq,true);
        ...
        Button reset=new Button(activity);PlayerStyle.button(reset);reset.setText("EQをフラットに戻す");body.addView(reset,new LinearLayout.LayoutParams(-1,dp(44)));effects.add(reset);
        reset.setOnClickListener(v->{applyCurve(0);preset.setSelection(0);});
        bass=check("EXTRA BASS風",initial.bass,true);amount=slider("強さ",100,initial.amount,true,v->save(),v->v+"% ");
        note("低音強調は本アプリの処理です。Sony独自の音質処理ではありません。補正中はピーク保護が働きます。まず小さめの音量でお試しください。");
