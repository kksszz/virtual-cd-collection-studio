...
public final class TrackAdapter extends BaseAdapter {
    ...
    public View getView(int p,View recycled,ViewGroup parent){Row row=recycled instanceof Row?(Row)recycled:new Row(context);MediaItem item=getItem(p);
        ...
        row.lyrics.setContentDescription(item.mediaMetadata.title+jp.virtualcd.player.LanguageStrings.text("：歌詞",": Lyrics"));
        row.lyrics.setOnClickListener(v->{if(context instanceof android.app.Activity activity)LyricsStore.show(activity,item);});
        row.star.setOnClickListener(v->favorite.accept(item));row.info.setContentDescription(item.mediaMetadata.title+jp.virtualcd.player.LanguageStrings.text("：曲のプロパティ",": Track properties"));row.info.setOnClickListener(v->properties.accept(item));return row;}
    private static final class Row extends LinearLayout implements Checkable {
        final TextView number,title;final Button lyrics,star;final ImageButton info;private boolean checked;
        Row(Context c){super(c);setGravity(Gravity.CENTER_VERTICAL);setMinimumHeight(PlayerStyle.dp(c,52));setPadding(PlayerStyle.dp(c,6),0,0,0);
            ...
            lyrics=new Button(c);lyrics.setFocusable(false);lyrics.setMinWidth(0);lyrics.setMinimumWidth(0);
            jp.virtualcd.player.ControlIcon.button(lyrics,"lyrics",jp.virtualcd.player.LanguageStrings.text("歌詞","Lyrics"));
            lyrics.setBackground(new android.graphics.drawable.RippleDrawable(android.content.res.ColorStateList.valueOf(0x3370cde6),null,null));
            addView(lyrics,new LayoutParams(PlayerStyle.dp(c,44),PlayerStyle.dp(c,44)));
            ...
            info.setTooltipText(jp.virtualcd.player.LanguageStrings.text("曲のプロパティ","Track properties"));addView(info,new LayoutParams(PlayerStyle.dp(c,44),PlayerStyle.dp(c,48)));setChecked(false);}
