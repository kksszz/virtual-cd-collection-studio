...
public final class ArtworkGallery {
    ...
    private void open(){
        ...
        caption.setText("画像一覧を読み込み中…");previous.setEnabled(false);next.setEnabled(false);
        worker.execute(()->{try{
            ...
            handler.post(()->{if(closed||activity.isDestroyed())return;images=found;
                if(images.isEmpty()){caption.setText("画像ファイルはありません（埋め込み画像は一覧対象外です）。");return;}load();});
        }catch(Exception e){handler.post(()->{if(!closed)caption.setText("画像一覧を読み込めません: "+e.getMessage());});}});
    }
    private void load(){
        loading=true;
        var entry=images.get(index);caption.setText((index+1)+" / "+images.size()+" · "+entry.name+"\n読み込み中…");
        ...
        worker.execute(()->{try{
            ...
            handler.post(()->{if(closed||activity.isDestroyed())return;image.setImageBitmap(bitmap);
                caption.setText((index+1)+" / "+images.size()+" · "+entry.name+(bitmap==null?"\n表示できない画像です":""));enableNavigation();});
