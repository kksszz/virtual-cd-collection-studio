...
public final class MobileSync {
    public static String safePath(String path)throws IOException{
        if(path.isEmpty()||path.contains("\\")||path.startsWith("/")||path.endsWith("/"))throw new IOException(jp.virtualcd.player.LanguageStrings.text("不正な同期パスです","Invalid sync path"));
        for(String part:path.split("/",-1))if(part.isEmpty()||part.equals(".")||part.equals(".."))throw new IOException(jp.virtualcd.player.LanguageStrings.text("不正な同期パスです","Invalid sync path"));
        return path;
    }
    private static class Resolver {
        ...
        AlbumLibrary.Album get(String path)throws Exception{
            ...
            for(String part:path.split("/")){
                ...
                var found=paths.get(parent+part);if(found==null)throw new IOException(jp.virtualcd.player.LanguageStrings.text("転送ファイルがありません: ","Transfer file is missing: ")+path);
    ...
    public static synchronized List<AlbumLibrary.Album> read(Context c,Uri root)throws Exception{
        return read(c,root,false);
    }
    private static synchronized List<AlbumLibrary.Album> read(Context c,Uri root,boolean forceFavorites)throws Exception{
        ...
        byte[] bytes;try(var in=c.getContentResolver().openInputStream(manifest.uri)){if(in==null)throw new IOException(jp.virtualcd.player.LanguageStrings.text("同期索引を開けません","Unable to open sync index"));bytes=CasePackage.readBytes(in,4*1024*1024);}
        ...
        while(keys.hasNext()){
            ...
            if(!id.matches("[0-9a-f]{64}")||!id.equals(record.getString("id")))throw new IOException(jp.virtualcd.player.LanguageStrings.text("不正なアルバムIDです","Invalid album ID"));
            ...
            if(!music.startsWith(prefix)||!glb.startsWith(prefix)||!hash.matches("[0-9a-f]{64}"))throw new IOException(jp.virtualcd.player.LanguageStrings.text("アルバムの対応情報が不正です","Invalid album mapping"));
            var audio=resolver.get(music);if(audio.directory!=record.getBoolean("directory"))throw new IOException(jp.virtualcd.player.LanguageStrings.text("音楽データの種類が不正です","Invalid audio data type"));
            audio=adoptExisting(c,root,record,audio,prefs);
            AlbumAddedOrder.bindSync(c,id,audio);
            var folder=new File(c.getFilesDir(),"cases3d");if(!folder.isDirectory()&&!folder.mkdirs())throw new IOException(jp.virtualcd.player.LanguageStrings.text("保存領域を作成できません","Unable to create storage"));
            ...
            if(!hash.equals(prefs.getString(binding,""))||!file.isFile()){
                var source=resolver.get(glb);byte[] data;try(var in=c.getContentResolver().openInputStream(source.uri)){if(in==null)throw new IOException(jp.virtualcd.player.LanguageStrings.text("3Dを開けません","Unable to open 3D data"));data=CasePackage.readBytes(in,CasePackage.MAX_BYTES);}
                if(!CasePackage.hash(data).equals(hash))throw new IOException(jp.virtualcd.player.LanguageStrings.text("3Dの転送検証に失敗しました","3D transfer verification failed"));
            ...
            if(record.has("lyrics")){
                String lyrics=record.getString("lyrics"),lyricsHash=record.getString("lyricsSha256");safePath(lyrics);
                if(!lyrics.startsWith(prefix)||!lyricsHash.matches("[0-9a-f]{64}"))throw new IOException(jp.virtualcd.player.LanguageStrings.text("歌詞の対応情報が不正です","Invalid lyrics mapping"));
                var source=resolver.get(lyrics);byte[] data;
                try(var in=c.getContentResolver().openInputStream(source.uri)){if(in==null)throw new IOException(jp.virtualcd.player.LanguageStrings.text("歌詞を開けません","Unable to open lyrics"));data=CasePackage.readBytes(in,LyricsStore.LIMIT);}
                if(!CasePackage.hash(data).equals(lyricsHash))throw new IOException(jp.virtualcd.player.LanguageStrings.text("歌詞の転送検証に失敗しました","Lyrics transfer verification failed"));
                LyricsStore.save(c,audio.uri,data);
            }
            FavoriteSync.receive(c,id,record,audio);
        ...
        FavoriteSync.applyCached(c,records,forceFavorites);
    ...
    public static synchronized List<AlbumLibrary.Album> refresh(Context c,Uri tree,List<AlbumLibrary.Album> current,boolean forceFavorites)throws Exception{
