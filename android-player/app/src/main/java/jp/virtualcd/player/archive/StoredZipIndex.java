...
public final class StoredZipIndex {
    ...
    private static ByteBuffer read(FileChannel channel, long offset, int length) throws IOException {
        if (offset < 0 || length < 0 || offset > channel.size() - length) throw new IOException("ZIP範囲が不正です");
        ByteBuffer b=ByteBuffer.allocate(length).order(ByteOrder.LITTLE_ENDIAN);
        while (b.hasRemaining()) {
            int n=channel.read(b, offset+b.position());
            if (n <= 0) throw new IOException("ZIPを読み取れません");
        }
        b.flip(); return b;
    }
    ...
    private static List<Entry> readEntries(FileChannel channel, boolean images) throws IOException {
        ...
        if (size < 22) throw new IOException("ZIPではありません");
        ...
        if (e<0) throw new IOException("ZIP終端情報がありません。PC側で無圧縮ZIPへ変換してください");
        if (u16(tail,e+4)!=0 || u16(tail,e+6)!=0 || u16(tail,e+8)!=u16(tail,e+10)) throw new IOException("分割ZIPは未対応です");
        int count=u16(tail,e+10);
        long central=u32(tail,e+16), centralSize=u32(tail,e+12);
        if (count==65535 || central==0xffffffffL || centralSize==0xffffffffL) throw new IOException("ZIP64は未対応です");
        if (central+centralSize>size-tailSize+e) throw new IOException("ZIPディレクトリ範囲が不正です");
        List<Entry> result=new ArrayList<>(); var names=new java.util.HashSet<String>(); long position=central;
        for (int i=0;i<count;i++) {
            if (Thread.currentThread().isInterrupted()) throw new IOException("読込を中止しました");
            ByteBuffer h=read(channel,position,46);
            if (u32(h,0)!=0x02014b50L) throw new IOException("ZIPディレクトリが壊れています");
            ...
            if (position>central+centralSize) throw new IOException("ZIPディレクトリが途切れています");
            ...
            if (!names.add(name)) throw new IOException("同名のMP3が重複しています");
            if(images && ((flags&1)!=0 || (method!=0&&method!=8))) continue;
            if (!images && ((flags&1)!=0 || method!=0)) throw new IOException("圧縮・暗号化された音声です。PC側で無圧縮ZIPに変換してください");
            long length=u32(h,24), localOffset=u32(h,42);
            long packed=u32(h,20);
            if (length==0xffffffffL || localOffset==0xffffffffL || packed==0xffffffffL || (method==0&&packed!=length)) throw new IOException("未対応のZIP形式です");
            ByteBuffer local=read(channel,localOffset,30);
            if (u32(local,0)!=0x04034b50L || u16(local,8)!=method || (u16(local,6)&1)!=0) throw new IOException("ZIPローカル情報が不正です");
            long offset=localOffset+30+u16(local,26)+u16(local,28);
            if (offset>central || packed>central-offset) throw new IOException("データ範囲が不正です");
            result.add(new Entry(name,offset,length,packed,method));
        }
        result.sort((a,b)->naturalKey(a.name).compareTo(naturalKey(b.name)));
        if (!images && result.isEmpty()) throw new IOException("ZIP内に対応音声（MP3/FLAC/WAV/M4A）がありません");
        return result;
    }
